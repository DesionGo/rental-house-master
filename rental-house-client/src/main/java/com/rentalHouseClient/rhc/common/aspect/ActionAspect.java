package com.rentalHouseClient.rhc.common.aspect;
...
import com.rentalHouseClient.rhc.modules.sys.entity.Log;
import com.rentalHouseClient.rhc.modules.sys.entity.clientUser.ClientUser;
import com.rentalHouseClient.rhc.modules.sys.service.ILogService;
...
public class ActionAspect {
	...
	@Pointcut("@annotation(com.rentalHouseClient.rhc.common.annotation.Log)")
	...
	private void saveActionLog(JoinPoint joinPoint, long time, boolean isAround) {
		ClientUser clientUser = (ClientUser) SecurityUtils.getSubject().getPrincipal();
		if (clientUser == null) {
			return;
		}
		MethodSignature signature = (MethodSignature) joinPoint.getSignature();
		...
		com.rentalHouseClient.rhc.common.annotation.Log action = method.getAnnotation(com.rentalHouseClient.rhc.common.annotation.Log.class);
		if(action != null){
			// 注解上的描述
		...
		// 设置用户信息
		actionLog.setUserId(clientUser.getId());
		actionLog.setUsername(clientUser.getUserName());
